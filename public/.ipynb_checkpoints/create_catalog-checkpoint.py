...
def create_regions(datas, num, fldconf):
    """
    Create annuli-regions aroud the center of each image. 
    """
    ...
    # We crop the Band 3 pointings to create non-overlapping catalogs later in the code, so we need to set the center point accordingly.
    if fldconf == 'NB3':
...
def get_radius(datas, cat, wcs_, fldconf):
    xc, yc = datas.shape[1]/2, datas.shape[1]/2
    if fldconf == 'NB3':
...
def catalog_for_configuration(fldconf, RMS = 5.0, num_regions = 7, min_delta_factor = 0.5, min_npix = 3):
    ...
    cat_coords, cat_final, IDs, concentric_ID = catalog_from_rings(dendros, wcs_, hdr, regions_rings, extended)
